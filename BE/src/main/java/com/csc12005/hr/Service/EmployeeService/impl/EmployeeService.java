...
public class EmployeeService implements IEmployeeService
{
	...
	@Override
    public List<EmployeeListItemResponse> getEmployeesForHr(Boolean status) {
        List<Employee> employees = (status == null)
                ? employeeRepository.findAll()
                : employeeRepository.findByStatus(status);

        // MapStruct mapper (method bạn đã thêm): toEmployeeListItemResponse
        return employees.stream()
                .map(employeeMapper::toEmployeeListItemResponse)
                .toList();
    }

    @Override
    public void deactivateEmployeeForHr(Long employeeId) {
        ...
        // Business rule: vô hiệu hóa đăng nhập
